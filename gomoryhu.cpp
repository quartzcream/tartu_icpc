...
class GomoryHu {
  ...
  void find_cut (int vertex, int source,
      ...
      if (comp[nxt] == source || comp[nxt] == nxt) {
        if (!partition[nxt]) {
          if (curcapac[vertex][nxt] > 0) {
            if (abs(mf.flow_on_edge(indices[vertex][nxt]))
                < curcapac[vertex][nxt]) {
              find_cut(nxt, source, partition,
                       curcapac, indices, mf);
            }
          }
        }
  ...
  void split_component (int vertex) {
    ...
    vector<vector<int>> newcapac
      (vertexc, vector<int> (vertexc, 0));
    for (int i = 0; i < vertexc; i++) {
    ...
    find_cut(sink, source, partition, newcapac,
             edgeIndices, mf);
    for (int i = 0; i < vertexc; i++) {
