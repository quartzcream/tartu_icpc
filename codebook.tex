...
struct node {
  ...
  node *suffix;
  node() { memset(nxt, 0, alpha_size * sizeof(node *)); }@ $ \hfill #364 $ @
  int cnt = 0;
};
node *aho_corasick(vector<vector<char> > &dict) {
  ...
  for (int i = 0; !state.empty(); ++i) {
    ...
    for (auto &cur : state) {@ $ \hfill #561 $ @
      node *nxt = cur.second->nxt[(*cur.first)[i]];
      if (nxt) {
        cur.second = nxt;
      } else {
        nxt = new node;@ $ \hfill #921 $ @
        ...
        while (suf) {@ $ \hfill #085 $ @
          ...
          suf = suf->suffix;@ $ \hfill #872 $ @
        }
      }
      if (cur.first->size() > i + 1) nstate.push_back(cur);
    }
    state = nstate;@ $ \hfill #505 $ @
  }
  return root;
}@ $ \hfill %823 $ @
...
node *walk(node *cur,
  ...
  while (true) {
    ...
    if (!cur->suffix) return cur;@ $ \hfill #113 $ @
    cur = cur->suffix;
  }
}@ $ \hfill %127 $ @
...
void add_cnt(node *root) { // After counting matches propagete ONCE to
  ...
  ran(i, 0, to_visit.size()) {
    node *cur = to_visit[i];
    ran(j, 0, alpha_size) {@ $ \hfill #797 $ @
      if (cur->nxt[j]) to_visit.push_back(cur->nxt[j]);
    }
  }
  for (int i = to_visit.size() - 1; i > 0; --i)
    to_visit[i]->suffix->cnt += to_visit[i]->cnt;@ $ \hfill #877 $ @
}@ $ \hfill %754 $ @
int main() {
  int n, len;
  scanf("%d %d", &len, &n);
...
\begin{minted}[tabsize=4,baselinestretch=1,linenos,numbersep = 1mm]{python}
def minimize():
	s = merge_all_loops()
	while size >= 3:
		t, u = find_pp()
