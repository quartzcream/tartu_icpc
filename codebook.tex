...
point ortho_center() { // euler line
  point O = circum_center();@ $ \hfill #895 $ @
...
point in_center() {
  ...
  double b = abs(A - C);
  double c = abs(A - B);@ $ \hfill #954 $ @
  // trilinear coordinates are (1,1,1)
  double sum = a + b + c;
  a /= sum;
  b /= sum;
  c /= sum;                     // barycentric
  return a * A + b * B + c * C; // cartesian$ \hfill #596 $
}@ $ \hfill %892 $ @
\end{minted}
...
Vec intersection(Seg l, Seg r) {@ $ \hfill #327 $ @
  Vec dl = l.d(), dr = r.d();
...
double calc_area(vector<Seg> lines) {
  ...
  for (auto line : lines) {@ $ \hfill #454 $ @
    if (line.b.y == line.a.y) {
      ...
      linesBySide[0].push_back({line.b, line.a});@ $ \hfill #613 $ @
    }
  }
  sort(
  ...
  sort(
    linesBySide[1].begin(), linesBySide[1].end(), [](Seg l, Seg r) {@ $ \hfill #115 $ @
      if (cross(l.d(), r.d()) == 0)
        return normal(l.d()) * l.a < normal(r.d()) * r.a;
      return cross(l.d(), r.d()) > 0;
    });
  ...
  for (int side = 0; side < 2; side++) {
    ...
    for (auto line : linesBySide[side]) {
      while (curLines.size() > 0) {@ $ \hfill #412 $ @
      ...
      if (curLines.size() == 0) {
        apply.push_back(-HUGE_VAL);@ $ \hfill #321 $ @
      } else {
  ...
  double result = 0;@ $ \hfill #908 $ @
  {
    ...
    for (int i = 0, j = 0; i < (int)linesBySide[0].size() &&
      ...
      if (cross(l1.d(), l0.d()) > 0) {
        alb = max(alb, intersection(l0, l1).y);@ $ \hfill #743 $ @
      ...
      aub = max(aub, alb);
      {
        ...
        result -= (aub - alb) * (x1 + x2) / 2;@ $ \hfill #075 $ @
      }
      {
...
struct Hull {
  ...
  void extend(It beg, It end) { // O(n)$ \hfill #096 $
    vector<Vec> r;
    for (auto it = beg; it != end; ++it) {
      if (r.empty() || *it != r.back()) {
        while (r.size() >= 2) {
          int n = r.size();@ $ \hfill #442 $ @
  ...
  Hull(vector<Vec> &vert) {         // atleast 2 distinct points
    ...
    int diff = hull.size();
    extend(vert.rbegin(), vert.rend());@ $ \hfill #964 $ @
  ...
  bool contains(Vec p) { // O(log(n))
    if (p < hull.front().F || p > up_beg->F) return false;
    {
      auto it_low = lower_bound(
        hull.begin(), up_beg, MP(MP(p.F, (int)-2e9), MP(0, 0)));@ $ \hfill #542 $ @
  ...
  SegIt max(function<T(Seg &)> f) { // O(log(n))
    auto l = hull.begin();
    auto r = hull.end();
    SegIt b = hull.end();@ $ \hfill #566 $ @
    T b_v;
    while (r - l > 2) {
      auto m = l + (r - l) / 2;
      T l_v = f(*l);
      T l_n_v = f(*(l + 1));@ $ \hfill #586 $ @
      T m_v = f(*m);
      T m_n_v = f(*(m + 1));
    ...
    T l_v = f(*l);
    if (b == hull.end() || l_v > b_v) {@ $ \hfill #864 $ @
      b = l;
      b_v = l_v;
    }
    if (r - l > 1) {
      T l_n_v = f(*(l + 1));@ $ \hfill #972 $ @
  ...
  Vec tan_point(Vec p) {   // can't be internal or on border
    ...
    auto best_seg = max(function<double(Seg &)>(
      [&p, &ref_p](@ $ \hfill #209 $ @
        ...
        Vec v2 = {seg.F.F - p.F, seg.F.S - p.S};
        ll d_p = dot(v1, v2);
        ll c_p = DIRECTION * cross(v2, v1);@ $ \hfill #762 $ @
  ...
  pair<SegIt, SegIt> intersections(Seg l) { // O(log(n))
    ...
    if (it_min <= it_max) {
      ...
      if (dot(dir, hull.front().F) <= opt_val) {
        ...
        it_r2 = upper_bound(it_min, hull.end(), l, inc_c) - 1;@ $ \hfill #220 $ @
      }
    }
...
struct Vec {
  ...
  bool operator!=(const Vec &oth) { return !operator==(oth); }@ $ \hfill #679 $ @
...
struct Edge {@ $ \hfill #732 $ @
  Vec tar;
  Edge *nxt;
  Edge *inv = NULL;
  Edge *rep = NULL;
  bool vis = false;@ $ \hfill #668 $ @
};
struct Seg {
  Vec a, b;
  bool operator==(const Seg &oth) { return a == oth.a && b == oth.b; }
  bool operator!=(const Seg &oth) { return !operator==(oth); }@ $ \hfill #245 $ @
};
...
bool in_c_circle(Vec *arr, Vec d) {
  ...
  return res > 0;@ $ \hfill #845 $ @
}@ $ \hfill %793 $ @
Edge *add_triangle(Edge *a, Edge *b, Edge *c) {
  Edge *old[] = {a, b, c};
  Edge *tmp = new Edge[3];
  ran(i, 0, 3) {
    old[i]->rep = tmp + i;@ $ \hfill #219 $ @
...
struct Node {
  ...
  Node *suffix;
  Node() { memset(nxt, 0, alpha_size * sizeof(Node *)); }@ $ \hfill #248 $ @
  int cnt = 0;
};
Node *aho_corasick(vector<vector<char> > &dict) {
  Node *root = new Node;
  root->suffix = 0;@ $ \hfill #292 $ @
  ...
  for (int i = 0; !state.empty(); ++i) {
    ...
    for (auto &cur : state) {@ $ \hfill #306 $ @
      Node *nxt = cur.second->nxt[(*cur.first)[i]];
      if (nxt) {
        cur.second = nxt;
      } else {
        nxt = new Node;@ $ \hfill #266 $ @
        ...
        while (suf) {@ $ \hfill #249 $ @
          ...
          suf = suf->suffix;@ $ \hfill #562 $ @
        }
      }
      if (cur.first->size() > i + 1) nstate.push_back(cur);
    }
    state = nstate;@ $ \hfill #417 $ @
  }
  return root;
}@ $ \hfill %882 $ @
...
Node *walk(Node *cur,
  ...
  while (true) {
    ...
    if (!cur->suffix) return cur;@ $ \hfill #414 $ @
    cur = cur->suffix;
  }
}@ $ \hfill %529 $ @
...
void add_cnt(Node *root) { // After counting matches propagete ONCE to
  ...
  ran(i, 0, to_visit.size()) {
    Node *cur = to_visit[i];
    ran(j, 0, alpha_size) {@ $ \hfill #662 $ @
      if (cur->nxt[j]) to_visit.push_back(cur->nxt[j]);
    }
  }
  for (int i = to_visit.size() - 1; i > 0; --i)
    to_visit[i]->suffix->cnt += to_visit[i]->cnt;@ $ \hfill #950 $ @
}@ $ \hfill %488 $ @
int main() {
  int n, len;
  scanf("%d %d", &len, &n);
...
class Node {
 ...
 public:@ $ \hfill #994 $ @
  ...
  Node *nxt(char c) {
    if (!has_nxt(c)) return NULL;@ $ \hfill #788 $ @
    return nxt_char[c];
  }
  void set_nxt(char c, Node *node) { nxt_char[c] = node; }
  Node *split(int new_len, char c) {
    Node *new_n = new Node;@ $ \hfill #449 $ @
    new_n->nxt_char = nxt_char;
    new_n->len = new_len;
    new_n->suf = suf;
    suf = new_n;
    return new_n;@ $ \hfill #130 $ @
  }@ $ \hfill %044 $ @
  // Extra functions for matching and counting
  Node *lower_depth(int depth) { // move to longest suffix of current
                                 // with a maximum length of depth.
    if (suf->len >= depth) return suf->lower_depth(depth);
    return this;
  }
  Node *walk(char c, int depth,@ $ \hfill #736 $ @
  ...
  int calc_end_dist() {
    if (end_dist == 1 << 30) {
      if (nxt_char.empty()) end_dist = 0;@ $ \hfill #524 $ @
...
struct SufAuto {
  ...
  void extend(char new_c) {
    Node *new_end = new Node;@ $ \hfill #340 $ @
    new_end->len = last->len + 1;
    Node *suf_w_nxt = last;
    while (suf_w_nxt && !suf_w_nxt->has_nxt(new_c)) {
      suf_w_nxt->set_nxt(new_c, new_end);
      suf_w_nxt = suf_w_nxt->suf;@ $ \hfill #217 $ @
    }
    if (!suf_w_nxt) {
      new_end->suf = root;
    } else {
      Node *max_sbstr = suf_w_nxt->nxt(new_c);@ $ \hfill #618 $ @
      if (suf_w_nxt->len + 1 == max_sbstr->len) {
        new_end->suf = max_sbstr;
      } else {
        Node *eq_sbstr = max_sbstr->split(suf_w_nxt->len + 1, new_c);
        new_end->suf = eq_sbstr;@ $ \hfill #295 $ @
        ...
        while (w_edge_to_eq_sbstr != 0 &&
          ...
          w_edge_to_eq_sbstr = w_edge_to_eq_sbstr->suf;@ $ \hfill #678 $ @
        }
      }
    }
    last = new_end;
...
struct MaxFlow {
  ...
  struct FlowTracker {
    ...
    ll cap, rcap;
    bool dir;
    FlowTracker(ll _cap, ll _rcap, shared_ptr<ll> _flow, int _dir)@ $ \hfill #418 $ @
  ...
  ll dinic_dfs(int u, ll flow) {
    if (u == sink) return flow;@ $ \hfill #725 $ @
  ...
  ll calc_max_flow() {
    ...
    while (dinic_bfs()) {
      ll cur = 0;
      do {
        cur = dinic_dfs(source, INF);@ $ \hfill #052 $ @
        ans += cur;
      } while (cur > 0);
    }
    return ans;
...
ll cost[nmax][nmax]; // set [x][y] for directed cost from x to y. SET$ \hfill #497 $
                     // TO inf IF NOT USED
ll min_dist[nmax];
int prev_node[nmax];
ll node_flow[nmax];
...
void min_cost_max_flow() {
  ...
  while (true) {
    ...
    while (true) { // Use Dijkstra to calculate potentials
      ...
      if (min_node == v) break visited[min_node] = true;@ $ \hfill #281 $ @
      for (int i = 0; i < v; ++i) {
        if ((!visited[i]) &&
    ...
    while (cur != 0) {
      ...
      if (rem_flow[prev_node[cur]][cur] == 0)@ $ \hfill #446 $ @
...
struct Network {
  ...
  Edge* addEdge(Node* u, Node* v, int f, int c, int cost) {@ $ \hfill #518 $ @
    ...
    while (1) {
        ...
        for (int i = 0; i < n; i++) {
          if (lev == 0 || state[lev][i].p < state[lev - 1][i].p) {
            for (Edge* edge : nodes[i].conn) {
              if (edge->getCap(&nodes[i]) > 0) {@ $ \hfill #760 $ @
                int np =
                  state[lev][i].p +
      ...
      for (int i = 0; i < n; i++)@ $ \hfill #283 $ @
        if (state[n - 1][i].p > state[n][i].p) {
          ...
          for (auto edge : path) {
            cap = min(cap, edge->getCap(cur));@ $ \hfill #297 $ @
            cur = edge->from(cur);
          }
          for (auto edge : path) {
...
struct DMST {
  ...
  struct Node {
    ...
    vector<pair<ll, Edge *> > con;@ $ \hfill #608 $ @
    bool in_use = false;
    ...
    Node *con_to_root() {@ $ \hfill #488 $ @
      ...
      if (super != root) { // we are some loops non first Node.
        if (con.size() > super->con.size()) {@ $ \hfill #860 $ @
          swap(con,
...
void DMST::Circle::clean(int idx) {
  if (!vis) {
    vis = true;
    for (int i = 0; i < contents.size(); ++i) {@ $ \hfill #814 $ @
      if (i != idx) {
        contents[i]->inc = true;
...
struct vert {
  ...
  int val = 0;
  int seen;
  int dfs(int upd, edge *ban) { // handles multiple edges$ \hfill #331 $
    if (!val) {
      val = upd;
      seen = val;
      for (edge &nxt : con) {
...
struct Graph {
  ...
  void _topsort_dfs(int pos, vector<int> &result, vector<bool> &explr,
    ...
    result.push_back(pos);@ $ \hfill #810 $ @
  }
  vector<int> topsort() {
    ...
    vector<bool> explr(n, false);
    for (int i = 0; i < n; i++)
      _topsort_dfs(i, result, explr, revconn);@ $ \hfill #178 $ @
  ...
  vector<vector<int> > scc() {
    ...
    vector<bool> explr(n, false);
    vector<vector<int> > results;@ $ \hfill #020 $ @
...
struct Seg {
  ...
  void recalc(const Seg &lhs_seg, int lhs_len, const Seg &rhs_seg,
    int rhs_len) {
    sum = lhs_seg.sum + rhs_seg.sum;@ $ \hfill #684 $ @
  }
...
struct Node { // Following code should not need to be modified
  ...
  Node *upd(int L, int R, int l, int r, Lazy &val, int tar_ver) {
    if (ver != tar_ver) {
      Node *rep = new Node(*this);@ $ \hfill #874 $ @
      rep->ver = tar_ver;
    ...
    if (L >= l && R <= r) {
      ...
      if (l < M) lc = lc->upd(L, M, l, r, l_val, ver);@ $ \hfill #045 $ @
...
class HLD {
  int vertexc;
  vector<int> *adj;
  vector<int> subtree_size;@ $ \hfill #178 $ @
  DS structure;
  DS aux;
  ...
  void build_hld(int vertex, int parent, int chain_source) {
    ...
    if (adj[vertex].size() > 1 ||
      ...
      build_hld(big_child, vertex, chain_source);@ $ \hfill #254 $ @
      for (int child : adj[vertex]) {
 ...
 public:
  HLD(int _vertexc) {
    vertexc = _vertexc;
    adj = new vector<int>[vertexc + 5];@ $ \hfill #800 $ @
  }
  ...
  void build(T initial) {
    ...
    par = vector<int>(vertexc + 5);@ $ \hfill #976 $ @
    cur = 0;
    build_sizes(1, -1);
    build_hld(1, -1, 1);
  ...
  T query_path(@ $ \hfill #325 $ @
    ...
    while (chain_root[u] != chain_root[v]) {
      if (ord[u] > ord[v]) {
        ...
        cur_id++;
        aux.set(cur_id, structure.query(ord[chain_root[v]], ord[v]));@ $ \hfill #485 $ @
        v = par[chain_root[v]];
      }
    }
    cur_id++;
 ...
 public:
  ...
  void initiate() {
    if (initiated) {@ $ \hfill #330 $ @
      ...
      positions.resize(unique(positions.begin(), positions.end()) -@ $ \hfill #822 $ @
                       positions.begin());
...
struct Treap {
  ...
  Node* merge(Node* left, Node* right) {
    ...
    Node* top;
    if (left->priority < right->priority) {@ $ \hfill #282 $ @
  ...
  void insert(int key, int value) {@ $ \hfill #918 $ @
  ...
  void erase(int key) {
    Node *left, *mid, *right;
    tie(left, mid) = split(key, root);@ $ \hfill #416 $ @
  ...
  long long sum_upto(int key, Node* cur) {
    ...
    if (key <= cur->key) {
      ...
      if (cur->lch) result += cur->lch->total;@ $ \hfill #122 $ @
      return result;
    }
  }
...
void msd_radixsort(
  ...
  const int msd_radix_lim = 100;@ $ \hfill #866 $ @
  const T mask = 255;
...
Complex operator*(const Complex &lft, const Complex &rgt) {@ $ \hfill #560 $ @
  return Complex{
...
void fft(vector<Complex> &arr, int ord, bool invert) {
  ...
  static int inc_pow = 1;@ $ \hfill #669 $ @
  ...
  if (inc_pow <= ord) {
    ...
    for (; inc_pow <= ord; ++inc_pow) {@ $ \hfill #517 $ @
      for (int idx_p = 0; idx_p < 1 << (ord - 1);
        ...
        if (is_inv) root_pow[idx].b = -root_pow[idx].b;@ $ \hfill #105 $ @
      }
    }
  }
  if (invert != is_inv) {
...
void mult_poly_mod(
  ...
  if (c.size() < 400) {@ $ \hfill #811 $ @
    ...
    for (int i = 0; i < min(b.size(), c.size()); ++i) {@ $ \hfill #528 $ @
      arr[0][i].b = b[i] & mask;
      arr[1][i].b = b[i] >> shift;
    }
    ...
    for (int i = 0; i < 2; ++i) {@ $ \hfill #644 $ @
      for (int j = 0; j < 2; ++j) {
        ...
        Complex mult = {0, -0.25};
        if (i ^ j) mult = {0.25, 0};
        for (int k = 0; k < (1 << fft_ord); ++k) {@ $ \hfill #983 $ @
    ...
    for (int i = 2; i < 4; ++i) {
      ...
      for (int k = 0; k < (int)c.size(); ++k) {@ $ \hfill #403 $ @
...
bool is_prime(ull n) { // n <= 1<<63, 1M rand/s
  ModArithm arithm(n);
  if (n == 2 || n == 3) return true;@ $ \hfill #104 $ @
  ...
  while (true) {@ $ \hfill #118 $ @
    ...
    while (true) {@ $ \hfill #290 $ @
      ...
      for (int j = 0; j < j_stop; ++j) {
        cur = arithm.multf(cur, abs(v_cur - lv));@ $ \hfill #468 $ @
...
map<ll, int> prime_factor(ll n,
  ...
  if (!res) {
    map<ll, int> res_act;
    for (int p : small_primes) {@ $ \hfill #770 $ @
      while (!(n % p)) {
        ++res_act[p];
        n /= p;
      }
...
static vector<K> berlekamp_massey(vector<K> ss) {
  vector<K> ts(ss.size());
  vector<K> cs(ss.size());
  cs[0] = K::unity;@ $ \hfill #349 $ @
  fill(cs.begin() + 1, cs.end(), K::zero);
  vector<K> bs = cs;
  int l = 0, m = 1;
  K b = K::unity;
  for (int k = 0; k < (int)ss.size(); k++) {@ $ \hfill #390 $ @
    K d = ss[k];
    assert(l <= k);
