...
//!escape Convex polygon algorithms
...
struct Hull {
  ...
  pair<SegIt, SegIt> intersections(Seg l) { // O(log(n))
    ...
    if (it_min <= it_max) {
      it_r1 =
        upper_bound(it_min, it_max + 1, l, inc_c) - 1;
      if (dot(dir, hull.front().F) >= opt_val) {
        it_r2 = upper_bound(
                  hull.begin(), it_min + 1, l, dec_c) -
                1;
      } else {
        it_r2 =
          upper_bound(it_max, hull.end(), l, dec_c) - 1;
      }
    } else {
      it_r1 =
        upper_bound(it_max, it_min + 1, l, dec_c) - 1;
      if (dot(dir, hull.front().F) <= opt_val) {
        it_r2 = upper_bound(
                  hull.begin(), it_max + 1, l, inc_c) -
                1;
      } else {
        it_r2 =
          upper_bound(it_min, hull.end(), l, inc_c) - 1;
      }
    }
...
bool intersects(Seg line,
  Seg seg) { // inclusive
  auto v1 =
    make_pair(line.S.F - line.F.F, line.S.S - line.F.S);
  auto v2 =
    make_pair(seg.F.F - line.F.F, seg.F.S - line.F.S);
  auto v3 =
    make_pair(seg.S.F - line.F.F, seg.S.S - line.F.S);
  ll val1 = cross(v1, v2);
  ll val2 = cross(v1, v3);
  return !(
    ...
    for (int i = 2; i < 200; ++i) {
      for (int t = 0; t < 100; ++t) {
        ...
        do {
          points.clear();
          ran(j, 0, i) {
            points.emplace_back(rand() % (2 * r) - r,
              rand() % (2 * r) - r);
          }
          ran(j, 0, i) {
        ...
        ran(j, 0, i) hull_dyn.insert_point(
          points[j].F, points[j].S);
        auto it1 = hull_dyn.rbegin();
        auto it2 = hull.up_beg;
        while (it1 != hull_dyn.rend() &&
               it2 != hull.hull.end()) {
          assert(it1->p.F == it2->F.F &&
                 it1->p.S == it2->F.S);
          ++it1;
          ++it2;
        }
        ...
        it2 = hull.hull.begin();
        assert(
          it1->p.F == it2->F.F && it1->p.S == it2->F.S);
        ++it1;
        assert(it1 == hull_dyn.rend());
        ran(j, 0, 10) {
          int x = rand() % 20 - 10;
  ...
  for (int r = 2; r < 1e9; r *= 2) {
    for (int i = 2; i < 200; ++i) {
      for (int t = 0; t < 100; ++t) {
        ...
        while (
          it1 != hull_dyn.end() && it2 != hull.up_beg) {
          assert(it1->p.F == it2->F.F &&
                 it1->p.S == it2->F.S);
          ++it1;
          ++it2;
        ...
        if (!hull.hull.empty()) {
          for (int j = 0; j < query.size(); ++j) {
            if (!hull.contains(query[j])) {
              ...
              best_dist =
                min(best_dist, dist(tmp.S, query[j]));
              auto v1 = make_pair(
                tmp.S.F - tmp.F.F, tmp.S.S - tmp.F.S);
              auto v2 = make_pair(query[j].F - tmp.F.F,
                query[j].S - tmp.F.S);
              auto v3 = make_pair(query[j].F - tmp.S.F,
                query[j].S - tmp.S.S);
...
int solve_brute(int n, vector<Vec> hull_points,
  ...
  for (int i = -100; i < 100; ++i) {
    for (int j = 0; j < 100; ++j) {
      ...
      int cur_not_inc = 0;
      for (auto it = all_points.begin(); it->S == 0;
           ++it) {
        ++cur_not_inc;
      }
      for (auto it = all_points.rbegin(); it->S == 0;
           ++it) {
        ++cur_not_inc;
      }
      max_not_inc = max(max_not_inc, cur_not_inc);
    ...
    for (int i = 1; i <= 10; ++i) {
      for (int j = 0; j <= 10; ++j) {
        for (int t = 0; t <= 10; ++t) {
          ...
          for (int k = 0; k < i; ++k) {
            hull_points.emplace_back(
              rand() % r, rand() % r);
          }
          vector<Vec> query_points;
          for (int k = 0; k < j; ++k) {
            query_points.emplace_back(
              rand() % r, rand() % r);
          }
          int ans = solve(n, hull_points, query_points);
          int brute_ans =
            solve_brute(n, hull_points, query_points);
      ...
      for (int t = 0; t < 100; ++t) {
        ...
        if (!hull.hull.empty()) {
          for (int j = 0; j < query1.size(); ++j) {
            auto best = hull.intersections(
              make_pair(query1[j], query2[j]));
            if (best.F == hull.hull.end()) {
              for (auto cur : hull.hull) {
                assert(!intersects(
                  make_pair(query1[j], query2[j]), cur));
              }
            } else {
              assert(intersects(
                make_pair(query1[j], query2[j]),
...
int main() {
  ...
  ran(i, 0, lim) {
    ...
    hull_dyn.insert_point(
      x, cord_offset - x * x - (rand() & 1));
  }
  ll res = rand();
  ran(i, 0, lim) {
    int x = rand() % (2 * cord_lim) - cord_lim;
