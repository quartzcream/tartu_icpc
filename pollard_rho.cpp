...
struct ModArithm {
  ...
  ull multf(ull a, ull b) {  // a, b in [0, min(2*n, 1<<63))
    ...
    __asm__(
        ...
        "neg %%cl;\n\t"
        "shlq %%cl, %%rdx;\n\t"
        "orq %%rdx, %%rax;\n\t"  // d = (a*b)/n = (abq_m >> shift) | (abq_h << (64 -shift)) //may be
                                 // smaller by one
        "mulq %[n];\n\t"         // d*n
        "subq %%rax, %[res];\n\t"  // ab_l - = dn_l
        : [res] "=&r"(res), [tmp] "=&r"(tmp)
        : [a] "%a"(a), [b] "rm"(b), [q] "r"(q), [shift] "rm"(shift), [n] "rm"(n)
        : "%rdx", "%cl", "cc");
  ...
  ull mult(ull a, ull b) { return fix(multf(a, b)); }
  //!finish
  //!start
  ull sqp1(ull a) { return multf(a, a) + 1; }
  //!finish
  //!start
  ull mults(ull a, ull b) { return (unsigned __int128)a * b % n; }
};
...
ll pollard_rho(ll n) {
  ...
  while (true) {
    ...
    while (true) {
      ll cur = 1;
      ll v_cur = v;
      for (int j = 0; j < cum_cnt && idx != tar; ++j) {
...
map< ll, int > prime_factor(ll n, map< ll, int > *res = NULL) {  // n <= 1<<61, 1500/s
  if (!res) {
    map< ll, int > res_act;
