...
struct Hull {
  ...
  Hull(vector<pair<int, int> > &vert) { // atleast 2 distinct points
    ...
    int diff = hull.size();
    extend_hull(vert.rbegin(), vert.rend());@ $ \hfill #011 $ @
    upper_begin = hull.begin() + diff;
  }@ $ \hfill %873 $ @
  bool contains(pair<int, int> p) { // O(log(n))
    if (p < hull.front().first || p > upper_begin->first)
      return false;
    {
      ...
      auto v1 =
        make_pair(it_low->second.first - it_low->first.first,@ $ \hfill #477 $ @
                  it_low->second.second - it_low->first.second);
      auto v2 = make_pair(p.first - it_low->first.first,
                          p.second - it_low->first.second);
    ...
    }
    {
      auto it_up = lower_bound(
        ...
        make_pair(make_pair(p.first, (int)2e9), make_pair(0, 0)));@ $ \hfill #129 $ @
      ...
      auto v2 = make_pair(p.first - it_up->second.first,
                          p.second - it_up->second.second);
      if (cross(v1, v2) > 0) // > 0 is inclusive, >=0 is exclusive
        return false;
    }
    return true;
  ...
  template <
    typename T> // The function can have only one local min and max
                ...
                max(function<
                    T(const pair<pair<int, int>, pair<int, int> > &)>
                      f) { // O(log(n))
    auto l = hull.begin();
    auto r = hull.end();
    vector<pair<pair<int, int>, pair<int, int> > >::iterator best =@ $ \hfill #479 $ @
      hull.end();
    T best_val;
    while (r - l > 2) {
      auto mid = l + (r - l) / 2;
      ...
      if (best == hull.end() ||
          l_val > best_val) { // If max is at l we may remove it from$ \hfill #200 $
                              // the range.
        best = l;
        best_val = l_val;
      }
  ...
  vector<pair<pair<int, int>, pair<int, int> > >::iterator closest(
    pair<int, int> p) { // p can't be internal(can be on border), hull
                        // must have atleast 3 points
    const pair<pair<int, int>, pair<int, int> > &ref_p =
      hull.front(); // O(log(n))
    return max(
      function<double(const pair<pair<int, int>, pair<int, int> > &)>(
        [&p,
         &ref_p](const pair<pair<int, int>, pair<int, int> >
                   &seg) { // accuracy of used type should be coord^-2
          if (p == seg.first) return 10 - M_PI;@ $ \hfill #900 $ @
          auto v1 = make_pair(seg.second.first - seg.first.first,
                              seg.second.second - seg.first.second);
          auto v2 = make_pair(p.first - seg.first.first,
                              p.second - seg.first.second);
          ll cross_prod = cross(v1, v2);
          if (cross_prod > 0) { // order the backside by angle
            auto v1 = make_pair(ref_p.first.first - p.first,
                                ref_p.first.second - p.second);
            auto v2 = make_pair(seg.first.first - p.first,
                                seg.first.second - p.second);@ $ \hfill #534 $ @
  ...
  pair<int, int> forw_tan(
    pair<int, int> p) { // can't be internal or on border
    const pair<pair<int, int>, pair<int, int> > &ref_p =
      hull.front(); // O(log(n))
    auto best_seg = max(
      function<double(const pair<pair<int, int>, pair<int, int> > &)>(
        [&p,
         &ref_p](const pair<pair<int, int>, pair<int, int> >
                   &seg) { // accuracy of used type should be coord^-2
          auto v1 = make_pair(ref_p.first.first - p.first,@ $ \hfill #089 $ @
                              ref_p.first.second - p.second);
          auto v2 = make_pair(seg.first.first - p.first,
                              seg.first.second - p.second);
          ll dot_prod = dot(v1, v2);
          ll cross_prod =
            cross(v2, v1); // cross(v1, v2) for back_tan !!!
  ...
  vector<pair<pair<int, int>, pair<int, int> > >::iterator max_in_dir(
    ...
    return max(
      function<ll(const pair<pair<int, int>, pair<int, int> > &)>(
        [&v](const pair<pair<int, int>, pair<int, int> > &seg) {
  ...
  intersections(
    pair<pair<int, int>, pair<int, int> > line) { // O(log(n))
    ...
    if (dot(dir, it_max->first) < opt_val ||
        dot(dir, it_min->first) > opt_val)@ $ \hfill #662 $ @
      return make_pair(hull.end(), hull.end());
    vector<pair<pair<int, int>, pair<int, int> > >::iterator it_r1,
      it_r2;
    function<bool(const pair<pair<int, int>, pair<int, int> > &,
                  const pair<pair<int, int>, pair<int, int> > &)>
      inc_comp(
        [&dir](const pair<pair<int, int>, pair<int, int> > &lft,
    ...
    if (it_min <= it_max) {
      ...
      if (dot(dir, hull.front().first) >= opt_val) {@ $ \hfill #826 $ @
        it_r2 =
      ...
      if (dot(dir, hull.front().first) <= opt_val) {
        it_r2 =
          upper_bound(hull.begin(), it_max + 1, line, inc_comp) - 1;@ $ \hfill #388 $ @
      } else {
  ...
  pair<pair<int, int>, pair<int, int> > diameter() { // O(n)
    ...
    auto v1 =
      make_pair(hull.back().second.first - hull.back().first.first,
                hull.back().second.second - hull.back().first.second);
    while (it2 != hull.begin()) {
      auto v2 =@ $ \hfill #919 $ @
        make_pair((it2 - 1)->second.first - (it2 - 1)->first.first,
    ...
    while (it2 != hull.end()) { // check all antipodal pairs
      if (dist_sq(it1->first, it2->first) > dia_sq) {
        ...
        dia_sq = dist_sq(res.first, res.second);@ $ \hfill #125 $ @
      }
      auto v1 = make_pair(it1->second.first - it1->first.first,
                          it1->second.second - it1->first.second);
      auto v2 = make_pair(it2->second.first - it2->first.first,
      ...
      if (decider == 0) { // report cross pairs at parallel lines.
        if (dist_sq(it1->second, it2->first) > dia_sq) {
          ...
          dia_sq = dist_sq(res.first, res.second);@ $ \hfill #860 $ @
        }
 ...
 public:
  ...
  AutoNode *walk(char c, int depth, int &match_len) { //move to longest suffix of walked path that is a substring
    ...
    if (has_nxt(c)) {	//as suffixes are in classes match\_len must be tracked externally
      ++match_len;
      return nxt(c)->lower_depth(depth);@ $ \hfill #227 $ @
    }
    if (suf)
...
struct SufAutomaton {
  ...
  void extend(char new_c) {
    ...
    while (suf_w_nxt && !suf_w_nxt->has_nxt(new_c)) {@ $ \hfill #308 $ @
...
public:
  ...
  void build (T initial) {@ $ \hfill #566 $ @
      ...
      locs_old[j+1] = locs_old[j]+bucket_sizes[j];
		...
		for(int j=0; j<256; ++j){
			if(locs[j+1]-locs[j] < msd_radix_lim){
				std::sort(locs[j], locs[j+1]);@ $ \hfill #018 $ @
        if(d & 1){
...
void fft(vector< Complex > &arr, int ord, bool invert) {
  ...
  if (inc_pow <= ord) {
    ...
    root_pow.resize(1 << ord);@ $ \hfill #710 $ @
...
void mult_poly_mod(vector< int > &a, vector< int > &b, vector< int > &c) {  // c += a*b
  ...
  if (c.size() < 400) {
    ...
    if (arr[0].size() != 1 << fft_ord)
      for (int i = 0; i < 4; ++i) arr[i].resize(1 << fft_ord);@ $ \hfill #811 $ @
