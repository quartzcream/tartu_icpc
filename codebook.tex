...
struct Hull {
  ...
  Hull(vector< pair< int, int > > &vert) {  // atleast 2 distinct points
    ...
    int diff = hull.size();
    extend_hull(vert.rbegin(), vert.rend());@ $ \hfill #979 $ @
  ...
  bool contains(pair< int, int > p) {  // O(log(n))
    if (p < hull.front().first || p > upper_begin->first) return false;
    {
      ...
      auto v1 = make_pair(it_low->second.first - it_low->first.first,
                          it_low->second.second - it_low->first.second);@ $ \hfill #992 $ @
  ...
  intersections(pair< pair< int, int >, pair< int, int > > line) {  // O(log(n))
    ...
    function< bool(const pair< pair< int, int >, pair< int, int > > &,
                   const pair< pair< int, int >, pair< int, int > > &) >
        dec_comp([&dir](const pair< pair< int, int >, pair< int, int > > &lft,@ $ \hfill #239 $ @
                        const pair< pair< int, int >, pair< int, int > > &rgt) {
          return dot(dir, lft.first) > dot(dir, rgt.first);
        });
    if (it_min <= it_max) {
...
long double getArea(vector<Segment> lines) {
	...
	for(auto line : lines) {
		if(line.b.y == line.a.y) {
			...
			linesBySide[1].push_back(line);@ $ \hfill #628 $ @
		else
	...
	sort(linesBySide[1].begin(), linesBySide[1].end(), [](Segment l, Segment r) {
		if(cross(l.d(), r.d()) == 0) return normal(l.d())*l.a < normal(r.d())*r.a;
		return cross(l.d(), r.d()) > 0;@ $ \hfill #095 $ @
	});
	...
	for(int side = 0; side < 2; side++) {
		...
		for(auto line : linesBySide[side]) {
			while(curLines.size() > 0) {
				...
				if(cross(line.d(), other.d()) != 0) {@ $ \hfill #442 $ @
	...
	{
		long double lb = -HUGE_VALL, ub;@ $ \hfill #130 $ @
		for(int i=0, j=0; i < (int)linesBySide[0].size() && j < (int)linesBySide[1].size();lb = ub) {
			...
			aub = min(aub, upperbound);@ $ \hfill #220 $ @
			aub = max(aub, alb);
			...
			}
			{
				...
				result += (aub - alb) * (x1 + x2) / 2;@ $ \hfill #354 $ @
			}
...
ll node_flow[nmax];
bool visited[nmax];
ll tot_cost, tot_flow; //output$ \hfill #165 $
void min_cost_max_flow(){
	tot_cost=0;							//Does not work with negative cycles.
	tot_flow=0;
	ll sink_pot=0;
  min_dist[0] = 0;@ $ \hfill %711 $ @
  ...
  sink_pot+=min_dist[v-1]; //Bellman-Ford end.$ \hfill %660 $
	while(true){
		...
		tot_flow+=node_flow[v-1];@ $ \hfill #835 $ @
...
struct Network {
	...
	Edge* addEdge(Node* u, Node* v, int f, int c, int cost) {
		...
		u->conn.push_back(&edges.back());@ $ \hfill #814 $ @
				...
				for(int i=0;i<n;i++)
					if(lev == 0 || state[lev][i].p < state[lev-1][i].p) {
						...
						for(Edge* edge : nodes[i].conn) if(edge->getCap(&nodes[i]) > 0) {
							...
							int ni = edge->from(&nodes[i])->index;@ $ \hfill #959 $ @
			...
			for(int i=0;i<n;i++)@ $ \hfill #217 $ @
				if(state[n-1][i].p > state[n][i].p) {
					...
					while(!explr[cur->index]) {
						...
						State cstate = state[clev][cur->index];@ $ \hfill #767 $ @
  ...
  void extend(char new_c) {
    ...
    if (!suf_w_nxt) {  // The new character isn't part of the old string
      ...
      if (suf_w_nxt->len + 1 == max_sbstr->len) { // Check whether splitting is needed
        ...
        new_end->suf = eq_sbstr;@ $ \hfill #154 $ @
...
public:
  ...
  int query (int left, int right) {
    cout << this << ' ' << left << ' ' << right << endl;@ $ \hfill #948 $ @
  }
};@ $ \hfill %932 $ @
  ...
  void build_hld (int vertex, int parent, int chain_source) {
    ...
    if (adj[vertex].size() > 1) {
      ...
      for (int child : adj[vertex]) {
        if ((child != parent) &&
            (subtree_size[child] > big_size)) {@ $ \hfill #467 $ @
          big_child = child;
...
public:
  ...
  void build (T initial) {
    ...
    build_sizes(1, -1);
    build_hld(1, -1, 1);
    structure = DS (vertexc + 5, initial);@ $ \hfill #954 $ @
    aux = DS (50, initial);
  }
  ...
  T query_path (int u, int v) { /* returns the "sum" of the path u->v */
    ...
    cur_id++;
    aux.set(cur_id, structure.query(min(ord[u], ord[v]), max(ord[u], ord[v])));@ $ \hfill #731 $ @
    ...
    T query (int qleft, int qright) {
      ...
      if (qleft == lend && qright == rend) {
        ...
        return comp()(left->query(qleft, qright),
                      right->query(qleft, qright));@ $ \hfill #815 $ @
      }
    }
  };
...
public:
  ...
  PersistentST (int _size, T initial) {
    ...
    for (int i = size - 1; i > 0; i--) {@ $ \hfill #942 $ @
  ...
  void set (int position, T _value) {
    ...
    roots.push_back(tree[1]);@ $ \hfill #685 $ @
  }
...
vector<complex<long double> > fastFourierTransform(vector<complex<long double> > a, bool inverse) {
	...
	for(int block = n; block > 1; block /= 2) {
		...
		for(int s=0; s < n; s += block)@ $ \hfill #644 $ @
	...
	for(int block = 2; block <= n; block *= 2) {
		...
		for(int s=0; s < n; s += block)
			for(int i=0;i<half; i++) {@ $ \hfill #599 $ @
...
struct Polynomial {
	...
	long double& operator[](int ind) {@ $ \hfill #163 $ @
		return a[ind];
	}
	...
	Polynomial operator-(Polynomial r) {return Polynomial(*this) -= r;}@ $ \hfill #782 $ @
	...
	Polynomial operator*(Polynomial r) {
		...
		fl = fastFourierTransform(fl, false);@ $ \hfill #634 $ @
...
struct Treap {
    ...
    pair<Node*, Node*> split(int key, Node* cur) {
        ...
        pair<Node*, Node*> result;@ $ \hfill #933 $ @
        if(key <= cur->key) {
    ...
    void insert(int key, int value) {
        ...
        pair<Node*, Node*> ret = split(key, root);@ $ \hfill #368 $ @
...
struct SegmentTree {
    ...
    long long get(int l, int r, int pos = 1, int lb = 0, int rb = -1) {
        ...
        long long result = 0;
        if(l < mid) {
            result += get(l, r, pos*2, lb, mid);@ $ \hfill #452 $ @
        }
        if(mid < r) {
...
int main() {
    ios_base::sync_with_stdio(false);@ $ \hfill #150 $ @
    cin.tie(0);
