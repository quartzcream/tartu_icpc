...
double calc_area(vector<Seg> lines) {
  ...
  for (int side = 0; side < 2; side++) {@ $ \hfill #3617 $ @
    ...
    linesBySide[side] = curLines;@ $ \hfill #1880 $ @
  }
...
class Node {
 ...
 public:@ $ \hfill #4994 $ @
  ...
  Node *walk(char c, int depth,@ $ \hfill #5736 $ @
...
int main() {
  ...
  scanf("%d %d %lld", &a, &b, &c);
  if(a != b){
  edges.emplace\_back(a, b, c, c); //(a, b, c, 0) for directed
  }
  }
  MaxFlow mf(edges);
  cout << mf.calc\_flow(1, n) << '\n';
...
struct DMST {
  ...
  DMST(int n, vector<EdgeDesc> &desc,
    ...
    for (int i = 0; i < n; ++i)@ $ \hfill #3693 $ @
      make_heap(graph[i].con.begin(), graph[i].con.end(), comp);
  }
  bool find() {
    root = cur_root;
    no_dmst = false;@ $ \hfill #8798 $ @
    for (auto &cur : graph) {
      cur.con_to_root();
      to_process.clear();
...
void DMST::Circle::clean(int idx) {
  if (!vis) {
    vis = true;
    for (int i = 0; i < contents.size(); ++i) {@ $ \hfill #8814 $ @
      if (i != idx) {
        contents[i]->inc = true;
...
bool DMST::no_dmst;@ $ \hfill #8417 $ @
DMST::Node *DMST::root;@ $ \hfill %0771 $ @
\end{minted}
...
struct Graph {
  ...
  vector<vector<int> > scc() {
    ...
    vector<bool> explr(n, false);
    vector<vector<int> > results;@ $ \hfill #7020 $ @
    for (auto it = order.rbegin(); it != order.rend(); ++it) {
      vector<int> component;
      _topsort_dfs(*it, component, explr, conn);
      sort(component.begin(), component.end());
      results.push_back(component);@ $ \hfill #2741 $ @
    }
    sort(results.begin(), results.end());
    return results;
  }
};@ $ \hfill #4983 $ @@ $ \hfill %4983 $ @
// Solution for:
