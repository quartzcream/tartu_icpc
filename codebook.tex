...
for ((i=0;i<${#starts[@]};i++)); do
  ...
  echo
done "//check last number in each block$ \hfill %298 $
\end{minted}
...
struct Hull {
  ...
  Hull(vector< pair< int, int > > &vert) {  // atleast 2 distinct points
    ...
    int diff = hull.size();
    extend_hull(vert.rbegin(), vert.rend());@ $ \hfill #789 $ @
  ...
  bool contains(pair< int, int > p) {  // O(log(n))
    if (p < hull.front().first || p > upper_begin->first) return false;
    {
      ...
      auto v1 = make_pair(it_low->second.first - it_low->first.first,
                          it_low->second.second - it_low->first.second);@ $ \hfill #867 $ @
  ...
  intersections(pair< pair< int, int >, pair< int, int > > line) {  // O(log(n))
    ...
    function< bool(const pair< pair< int, int >, pair< int, int > > &,
                   const pair< pair< int, int >, pair< int, int > > &) >
        dec_comp([&dir](const pair< pair< int, int >, pair< int, int > > &lft,@ $ \hfill #933 $ @
                        const pair< pair< int, int >, pair< int, int > > &rgt) {
          return dot(dir, lft.first) > dot(dir, rgt.first);
        });
    if (it_min <= it_max) {
...
long double getArea(vector<Segment> lines) {
	...
	for(int side = 0; side < 2; side++) {
		...
		for(auto line : linesBySide[side]) {
			...
			curLines.push_back(line);@ $ \hfill #041 $ @
		}
	...
	long double result = 0;
	{
		...
		for(int i=0, j=0; i < (int)linesBySide[0].size() && j < (int)linesBySide[1].size();lb = ub) {
			ub = min(applyStart[0][i+1], applyStart[1][j+1]);@ $ \hfill #751 $ @
			...
			if(applyStart[0][i+1] < applyStart[1][j+1])@ $ \hfill #443 $ @
				i++;
			else
				j++;
		}
...
ll node_flow[nmax];
bool visited[nmax];
ll tot_cost, tot_flow; //output$ \hfill #660 $
void min_cost_max_flow(){
	tot_cost=0;							//Does not work with negative cycles.
	tot_flow=0;
	ll sink_pot=0;
  min_dist[0] = 0;@ $ \hfill %004 $ @
  ...
  sink_pot+=min_dist[v-1]; //Bellman-Ford end.$ \hfill %019 $
	while(true){
		...
		tot_flow+=node_flow[v-1];@ $ \hfill #854 $ @
				...
				for(int i=0;i<n;i++)
					if(lev == 0 || state[lev][i].p < state[lev-1][i].p) {
						...
						for(Edge* edge : nodes[i].conn) if(edge->getCap(&nodes[i]) > 0) {
							...
							if(np < state[lev+1][ni].p) {@ $ \hfill #963 $ @
								state[lev+1][ni].p = np;
			...
			for(int i=0;i<n;i++)
				if(state[n-1][i].p > state[n][i].p) {
					...
					while(!explr[cur->index]) {
						...
						path.push_back(cstate.used);@ $ \hfill #873 $ @
					}
    ...
    if (!suf_w_nxt) {  // The new character isn't part of the old string
      ...
      if (suf_w_nxt->len + 1 == max_sbstr->len) { // Check whether splitting is needed
        ...
        while (w_edge_to_eq_sbstr != 0 && w_edge_to_eq_sbstr->nxt(new_c) == max_sbstr) {
          ...
          w_edge_to_eq_sbstr = w_edge_to_eq_sbstr->suf;@ $ \hfill #256 $ @
        }
      }
    }
    last = new_end;
...
public:
  ...
  T query_path (int u, int v) { /* returns the "sum" of the path u->v */
    ...
    cur_id++;
    aux.set(cur_id, structure.query(min(ord[u], ord[v]), max(ord[u], ord[v])));@ $ \hfill #379 $ @
...
struct Polynomial {
	...
	Polynomial operator/(long double r) {return Polynomial(*this) /= r;}@ $ \hfill #211 $ @
