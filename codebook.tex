...
\setlength\columnsep{10mm}
\begin{multicols*}{2}
\section{gcc ordered set, hashtable}
...
using namespace std;@ $ \hfill %1386 $ @
...
template <typename T>@ $ \hfill #1392 $ @
using ordered_set = tree<T, null_type, less<T>, rb_tree_tag,
  tree_order_statistics_node_update>;@ $ \hfill %3907 $ @
int main() {
  ordered_set<int> cur;
  cur.insert(1);
  cur.insert(3);
...
\end{minted}
\section{PRNGs and Hash functions}
\begin{minted}[tabsize=2,baselinestretch=1,linenos,numbersep = 1mm, breaklines, frame=lines, escapeinside=@@, texcomments=true, mathescape=true]{c++}
mt19937 gen;
uint64_t rand64() { return gen() ^ ((uint64_t)gen() << 32); }@ $ \hfill %4798 $ @
uint64_t rand64() {
...
Vec sub(const Vec &v1, const Vec &v2) {@ $ \hfill #7360 $ @
...
ll dist_sq(const Vec &p1, const Vec &p2) {
  return (ll)(p2.F - p1.F) * (p2.F - p1.F) +@ $ \hfill #3379 $ @
...
struct Point {
  ...
  typename multiset<Point>::iterator get_it() const {
    tuple<void *> tmp = {(void *)this - 32}; // gcc rb\_tree dependent
    return *(multiset<Point>::iterator *)(&tmp);@ $ \hfill #7198 $ @
  }
  bool operator<(const Point &rhs) const {
    return (p.F < rhs.p.F); // sort by x
  }
  bool operator<(const Vec &q) const {@ $ \hfill #2890 $ @
...
struct HullDynamic : public multiset<Point, less<> > {
  bool bad(iterator y) {
    if (y == begin()) return 0;@ $ \hfill #5204 $ @
    auto x = prev(y);
    auto z = next(y);
  ...
  ll eval(int x) {    // O(log(N)) upper maximize dot({x, 1}, v)
    ...
    auto it = lower_bound((Vec){x, part});@ $ \hfill #4908 $ @
...
};@ $ \hfill #1268 $ @@ $ \hfill %1268 $ @
struct SufAuto {
  Node *last;
  Node *root;
  void extend(char new_c) {
    Node *nlast = new Node;@ $ \hfill #0936 $ @
    nlast->len = last->len + 1;
    Node *swn = last;
    while (swn && !swn->has_nxt(new_c)) {
      swn->set_nxt(new_c, nlast);
      swn = swn->suf;@ $ \hfill #1831 $ @
    }
    if (!swn) {
      nlast->suf = root;
    } else {
      Node *max_sbstr = swn->nxt(new_c);@ $ \hfill #0855 $ @
      if (swn->len + 1 == max_sbstr->len) {
        nlast->suf = max_sbstr;
      } else { // remove for minimal DFA that matches suffixes and
               // crap
        Node *eq_sbstr = max_sbstr->split(swn->len + 1, new_c);
        nlast->suf = eq_sbstr;@ $ \hfill #1749 $ @
        Node *x = swn; // x = with\_edge\_to\_eq\_sbstr
...
\end{minted}
\section{Splay Tree + Link-Cut $O(N\log{N})$}
\begin{minted}[tabsize=2,baselinestretch=1,linenos,numbersep = 1mm, breaklines, frame=lines, escapeinside=@@, texcomments=true, mathescape=true]{c++}
struct Tree *treev;
struct Tree {
  struct T {
    int i;
    constexpr T() : i(-1) {}@ $ \hfill #7635 $ @
    T(int _i) : i(_i) {}
...
T splay(T n) {
  for (;;) {
    propagate(n);@ $ \hfill #3792 $ @
    T p = n->p;
    if (p == NIL) break;
    propagate(p);
    ll px = p->c[1] == n;
    assert(p->c[px] == n);@ $ \hfill #0245 $ @
    T g = p->p;
    if (g == NIL) { /* zig */
      p->c[px] = n->c[px ^ 1];
      p->c[px]->p = p;
      n->c[px ^ 1] = p;@ $ \hfill #4750 $ @
      n->c[px ^ 1]->p = n;
      n->p = NIL;
      update(p);
      update(n);
    ...
    if (gx == px) { /* zig zig */
      g->c[gx] = p->c[gx ^ 1];
      g->c[gx]->p = g;@ $ \hfill #5608 $ @
      p->c[gx ^ 1] = g;
      p->c[gx ^ 1]->p = p;
      p->c[gx] = n->c[gx ^ 1];
      p->c[gx]->p = p;
      n->c[gx ^ 1] = p;@ $ \hfill #1439 $ @
      n->c[gx ^ 1]->p = n;
    } else { /* zig zag */
      g->c[gx] = n->c[gx ^ 1];
      g->c[gx]->p = g;
      n->c[gx ^ 1] = g;@ $ \hfill #9140 $ @
      n->c[gx ^ 1]->p = n;
      p->c[gx ^ 1] = n->c[gx];
      p->c[gx ^ 1]->p = p;
      n->c[gx] = p;
      n->c[gx]->p = n;@ $ \hfill #2928 $ @
    }
    if (gg) gg->c[ggx] = n;
    n->p = gg;
    update(g);
...
T set_child(T t, int x, T a) {@ $ \hfill #2821 $ @
  T o = t->c[x];
  t->c[x] = a;
  update(t);
  o->p = NIL;
...
void make_root(T t) {
  expose(t);
  lazy_reverse(extreme(splay(t), 0));@ $ \hfill #4240 $ @
}
/*rp*/@ $ \hfill %8430 $ @
\end{minted}
