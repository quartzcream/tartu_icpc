...
\end{minted}
\section{Generic segment tree(lazy, noncommutative)}
\begin{minted}[tabsize=2,baselinestretch=1,linenos,numbersep = 1mm, breaklines, frame=lines, escapeinside=@@, texcomments=true, mathescape=true]{c++}
struct Segment{
	ll sum_val=0;
	ll min_val=0;
	...
	void recalc(int seg_len, const Segment &lhs_seg, const Segment &rhs_seg){@ $ \hfill #599 $ @
...
struct Lazy{
	...
	void init(){
		add_val = 0;
		assign_val = LLONG_MIN;@ $ \hfill #577 $ @
	}
	Lazy(){ init(); }
  void apply_to_lazy(int seg_len, Lazy &child) const{
	...
	void apply_to_seg(int seg_len, Segment &cur) const{@ $ \hfill #523 $ @
		if(assign_val != LLONG_MIN){
	...
	} //Following code should not need to be modified$ \hfill %992 $
...
private:
  ...
  void split(int len, int idx){
    ...
    is_lazy[2*idx] = true;
    is_lazy[2*idx+1] = true;@ $ \hfill #984 $ @
  }
	...
	void build(int len, int idx){@ $ \hfill #201 $ @
		for(; idx; len<<=1, idx>>=1){
...
public:
	SegTree(int tree_size){
		...
		lazys = new Lazy[2*tree_size];@ $ \hfill #920 $ @
	...
	void modify(int l, int r, const Lazy &upd){
		...
		l >>=  __builtin_ctz(l) + 1;
		build(len, l);
		len = 1<<(__builtin_ctz(r)+1);@ $ \hfill #339 $ @
		r >>= __builtin_ctz(r) + 1;
		build(len, r);
	}
...
void fft(vector<Complex> &arr, int ord, bool invert){@ $ \hfill #689 $ @
  ...
  if(inc_pow <= ord){
    ...
    for(; inc_pow<=ord; ++inc_pow){
      for(int idx_p = 0; idx_p < 1<<(ord-1); idx_p += 1<<(ord-inc_pow), ++idx){
        root_pow[idx] = Complex {cos(-idx_p*M_PI/(1<<(ord-1))), sin(-idx_p*M_PI/(1<<(ord-1)))};@ $ \hfill #830 $ @
        if(is_inv)
          root_pow[idx].b = -root_pow[idx].b;
      }
    }
  }
  if(invert != is_inv){
    is_inv = invert;
    for(Complex &cur : root_pow)
      cur.b = -cur.b;
  }@ $ \hfill #717 $ @
  int rev = 0;
  ...
  fft_rec(arr.data(), root_pow.data(), 1<<(ord-1));@ $ \hfill #351 $ @
  if(invert)
...
void mult_poly_mod(vector<int> &a, vector<int> &b, vector<int> &c){ // c += a*b
  ...
  if(c.size() < 400){
    ...
    if((int)arr[0].size() < 1<<fft_ord)
      for(int i=0; i<7; ++i)@ $ \hfill #082 $ @
        arr[i].resize(1<<fft_ord);
    for(int i=0; i<7; ++i)
    ...
    for(int &cur : b)
      if(cur < 0)
        cur += mod;
    const int shift = 15;@ $ \hfill #019 $ @
    const int mask = (1<<shift)-1;
    for(int i=0; i < (int)min(a.size(), c.size()); ++i){
      arr[0][i].a = a[i]&mask;
      arr[1][i].a = a[i]>>shift;
    }
    for(int i=0; i < (int)min(b.size(), c.size()); ++i){
      arr[2][i].a = b[i]&mask;
      arr[3][i].a = b[i]>>shift;
    }
    for(int i=0; i<4; ++i)@ $ \hfill #432 $ @
      fft(arr[i], fft_ord, false);
    for(int i=0; i<2; ++i){
      for(int j=0; j<2; ++j){
        int tar = 4+i+j;
    ...
    for(int i=4; i<7; ++i){
      fft(arr[i], fft_ord, true);@ $ \hfill #272 $ @
...
bool is_prime(ll n) { //guaranteed for 64 bit numbers$ \hfill #280 $
  ...
  for (__int128 a : witnesses) {
    if (a >= mod) break;
    a = pow_mod(a, d, mod);
    if (a == 1 || a == mod - 1) continue;@ $ \hfill #667 $ @
    for (ll r = 1; r < s; ++r) {
      a = a * a % mod;
      if (a == 1) return false;
      if (a == mod - 1) break;
