...
struct ModArithm {
  ...
  ull multf(ull a, ull b) {  // a, b in [0, min(2*n, 1<<63))
    ...
    __asm__(
        ...
        "subq %%rax, %[res];\n\t"  // ab_l - = dn_l
        : [res] "=&r"(res), [tmp] "=&r"(tmp)
        : [a] "%a"(a), [b] "rm"(b), [q] "r"(q), [shift] "rm"(shift), [n] "rm"(n)@ $ \hfill #499 $ @
        : "%rdx", "%cl", "cc");
  ...
  ull mult(ull a, ull b) { return fix(multf(a, b)); }@ $ \hfill %440 $ @
  ull sqp1(ull a) { return multf(a, a) + 1; }@ $ \hfill %880 $ @
...
ll pollard_rho(ll n) {
  ...
  while (true) {
    ...
    while (true) {@ $ \hfill #896 $ @
      ...
      cur = arithm.fix(cur);
      if (!cur) {
        for (int j = 0; j < cum_cnt; ++j) {@ $ \hfill #866 $ @
