...
struct MaxFlow {
  ...
  MaxFlow(
    ...
    rcap.resize(start.back());/*ly*/
    cost.resize(start.back());/*ry*/
    for (auto &cur : edges) {
      int u, v;
  ...
  bool recalc_dist(bool check_imp = false) {
    ...
    while (!dist_que.empty()) {
      ...
      if (!visited[u]) {
        visited[u] = true;/*lp*/
        if (check_imp && dist != 0) imp = true;/*rp*/
  ...
  /*ly*/pair<ll, /*ry*/ll/*ly*/>/*ry*/ calc_flow(int _source, int _sink) {
    source = _source;
    sink = _sink;
    assert(max(source, sink) < start.size() - 1);/*ly*/
    ll tot_flow = 0;
    ll tot_cost = 0;/*lp*/
    if (recalc_dist_bellman_ford()) {
      assert(false);
    } else {/*rp*/
        while (recalc_dist()){/*ry*/
        ll flow = 0;
