...
\end{minted}
\section{Rabbin Miller prime check, Pollard rho factorization $\mathcal{O}(\sqrt{p})$}
\begin{minted}[tabsize=2,baselinestretch=1,linenos,numbersep = 1mm, breaklines, frame=lines, escapeinside=@@, texcomments=true, mathescape=true]{c++}
...
bool is_prime(ll n) { //guaranteed for 64 bit numbers
  ...
  for (__int128 a : small_primes) {
    if (a >= mod) break;
    a = pow_mod(a, d, mod);
    if (a == 1 || a == mod - 1) continue;@ $ \hfill #754 $ @
    for (ll r = 1; r < s; ++r) {
      a = a * a % mod;
      if (a == 1) return false;
      if (a == mod - 1) break;
...
ll pollard_rho(ll n){
  while(true){
    ...
    int tar = 1;
    for(int i=1; ; ++i){
      ll g = __gcd(abs(v-lv), n);@ $ \hfill #695 $ @
      if(g == n){
        break;
      } else if(g != 1){
        return g;
...
void prime_factor(ll n, map<ll, int> &res){
  ...
  if(n != 1){
    if(is_prime(n)){
      ...
      prime_factor(factor, res);@ $ \hfill #850 $ @
