...
struct DMST{
    ...
    Node *con_to_root(){
      ...
      while(super == this){
        ...
        if(nxt.second->tar->in_use){ //anc() wouldn't change anything
          super = nxt.second->tar->anc();@ $ \hfill #646 $ @
        ...
        if(super != root){
          ...
          nxt.second->inc = true;
          nxt.second->from->clean();@ $ \hfill #576 $ @
        }
      }
      if(super != root){ //we are some loops non first Node.
        ...
        for(auto cur : con){
          ...
          push_heap(super->con.begin(), super->con.end(), comp);@ $ \hfill #594 $ @
        }
      }
  ...
  vector<Node> graph;
  vector<Edge> edges;
  DMST(int n, vector<EdgeDesc> &desc, int r){ //Self loops and multiple edges are okay.$ \hfill #940 $
    graph.resize(n);
    cur_root = &graph[r];
...
struct Graph {
	...
	vector<vector<int> > scc(){ // tested on https://www.hackerearth.com/practice/algorithms/graphs/strongly-connected-components/practice-problems/algorithm/a-walk-to-remember-qualifier2/
		...
		for(auto it = order.rbegin(); it != order.rend(); ++it){
			...
			results.push_back(component);@ $ \hfill #522 $ @
		}
...
struct Lazy{
	...
	void init(){
		add_val = 0;
		assign_val = LLONG_MIN;@ $ \hfill #237 $ @
	}
	Lazy(){ init(); }
  void apply_to_lazy(int seg_len, Lazy &child) const{
	...
	void apply_to_seg(int seg_len, Segment &cur) const{@ $ \hfill #242 $ @
		if(assign_val != LLONG_MIN){
	...
	} //Following code should not need to be modified$ \hfill %047 $
...
public:
	SegTree(int tree_size){
		...
		segs = new Segment[2*tree_size];@ $ \hfill #042 $ @
